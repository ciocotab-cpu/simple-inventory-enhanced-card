    ...
    <div class="item-edit-form" style="max-width: 650px; width: 95%;">
      ...
      <div style="display: flex; flex-direction: column; gap: 14px;">
        ...
        <div style="display: flex; gap: 12px; width: 100%;">
          ...
          <div class="edit-field" style="flex: 1;"><label>${lang.lbl_price}</label><input type="text" id="edit_price" value="${item.price || ''}" placeholder="${lang.lbl_price_placeholder}"></div>
          <div class="edit-field" style="flex: 1;"><label>${lang.lbl_barcode}</label><input type="text" id="edit_barcode" value="${currentBarcode}"></div>
        </div>
        
        <!-- AREA UNIFICATA LISTA SPESA (Con la nuova select To-Do sincronizzata) -->
        <div style="display: flex; flex-direction: column; gap: 12px; padding: 12px; background: var(--secondary-background-color); border-radius: 8px; border: 1px solid var(--divider-color);">
          <div style="display: flex; align-items: center; gap: 10px;">
            <input type="checkbox" id="edit_auto_add_checkbox" ${item.auto_add_enabled === true ? 'checked' : ''} style="width: 20px; height: 20px; cursor: pointer;">
            <label for="edit_auto_add_checkbox" style="font-weight: 600; font-size: 0.9rem; cursor: pointer; color: var(--primary-text-color);">${lang.lbl_auto_add}</label>
          </div>
          
          <div id="edit_auto_add_subrow" style="display: flex; flex-direction: row; gap: 12px; width: 100%; transition: opacity 0.2s; opacity: ${item.auto_add_enabled === true ? '1' : '0.5'};">
            <div class="edit-field" style="flex: 1;"><label>${lang.lbl_min_qty}</label><input type="number" id="edit_min_qty" value="${item.auto_add_to_list_quantity !== undefined ? item.auto_add_to_list_quantity : 2}" ${item.auto_add_enabled === true ? '' : 'disabled'} placeholder="${lang.lbl_min_qty_placeholder}"></div>
            
            <div class="edit-field" style="flex: 1;">
              <label>${lang.lbl_todo}</label>
              <select id="edit_todo" ${item.auto_add_enabled === true ? '' : 'disabled'} style="width: 100%; padding: 6px 8px; border: 1px solid var(--divider-color); border-radius: 6px; background: var(--card-background-color); color: var(--primary-text-color); height: 34px;">
                <option value="">${lang.lbl_todo_placeholder}</option>
                ${todoLists.map(t => `${t.name}</option>`).join('')}
              </select>
            </div>
            
            <div class="edit-field" style="flex: 1;"><label>${lang.lbl_todo_placement}</label><select id="edit_todo_placement" ${item.auto_add_enabled === true ? '' : 'disabled'} style="width: 100%; padding: 6px 8px; border: 1px solid var(--divider-color); border-radius: 6px; background: var(--card-background-color); color: var(--primary-text-color); height: 34px;"><option value="name" ${item.todo_quantity_placement === 'name' ? 'selected' : ''}>${lang.opt_placement_name}</option><option value="description" ${item.todo_quantity_placement === 'description' ? 'selected' : ''}>${lang.opt_placement_desc}</option></select></div>
          </div>
        </div>
      </div>
      <div class="edit-form-actions" style="margin-top: 16px; border-top: 1px solid var(--divider-color); padding-top: 12px;">
...
export function getAddPopupHtml(lang, existingCategories = [], dataObj = {}) {
  ...
  return `
    <div class="item-add-popup" style="max-width: 650px; width: 95%;">
      <!-- STILE CSS INTEGRATO: Gestione dei placeholder -->
      <style>
        select.placeholder-style { color: var(--secondary-text-color, #727272) !important; }
        select.placeholder-style option { color: var(--primary-text-color) !important; }
        select.placeholder-style option:first-child { color: var(--secondary-text-color, #727272) !important; }
        input[type="date"].placeholder-style { color: var(--secondary-text-color, #727272) !important; }
        input[type="date"].placeholder-style:focus, input[type="date"].placeholder-style:valid { color: var(--primary-text-color) !important; }
      </style>
      ...
      <div style="display: flex; flex-direction: column; gap: 14px;">
        <div style="display: flex; gap: 12px; align-items: flex-end; width: 100%;">
          ...
          <div class="edit-field" style="flex: 3;"><label>${lang.field_qty}</label>
            <div style="display: flex; align-items: center; border: 1px solid var(--divider-color); border-radius: 6px; overflow: hidden; background: var(--card-background-color); height: 38px;">
              <button id="add-qty-dec" style="background: transparent; border: none; color: var(--primary-text-color); font-weight: bold; width: 32px; height: 100%; cursor: pointer; font-size: 1.2rem;">-</button>
              ...
              <button id="add-qty-inc" style="background: transparent; border: none; color: var(--primary-text-color); font-weight: bold; width: 32px; height: 100%; cursor: pointer; font-size: 1.2rem;">+</button>
            </div>
          </div>
        </div>
        <div style="display: grid !important; grid-template-columns: repeat(2, 1fr) !important; gap: 12px !important; width: 100% !important;">
          ...
          <div class="edit-field" style="width: 100%;">
            <label>${lang.lbl_cat}</label>
            <select id="new_cat_select" class="placeholder-style" onchange="this.className = this.value === '' ? 'placeholder-style' : ''" style="width: 100%; padding: 6px 8px; border: 1px solid var(--divider-color); border-radius: 6px; background: var(--card-background-color); color: var(--primary-text-color); font-size: 0.85rem; height: 34px; outline: none; cursor: pointer; font-family: inherit; box-sizing: border-box;">
              <option value="">${lang.lbl_cat_placeholder}</option>
              ${catOptions}
              <option value="__NEW_CAT__">${lang.lbl_cat_new_cat}</option>
            </select>
        ...
        <div style="display: flex; gap: 12px; width: 100%;">
          <div class="edit-field" style="flex: 1;"><label>${lang.lbl_expiry}</label><input type="date" id="new_expiry" class="placeholder-style" required oninput="this.className = this.value === '' ? 'placeholder-style' : ''"></div>
          <div class="edit-field" style="flex: 1;"><label>${lang.lbl_exp_alert}</label><input type="number" id="new_exp_alert" placeholder="0"></div>
        </div>
        <div style="display: flex; gap: 12px; width: 100%;">
        ...
        <div style="display: flex; gap: 12px; width: 100%;">
          ...
          <div class="edit-field" style="flex: 1;"><label>${lang.lbl_price}</label><input type="text" id="new_price" placeholder="${lang.lbl_price_placeholder}"></div>
          <div class="edit-field" style="flex: 1;"><label>${lang.lbl_barcode}</label><input type="text" id="new_barcode"></div>
        </div>
        <div style="display: flex; flex-direction: column; gap: 12px; padding: 12px; background: var(--secondary-background-color); border-radius: 8px; border: 1px solid var(--divider-color);">
          <div style="display: flex; align-items: center; gap: 10px;">
            <input type="checkbox" id="new_auto_add_checkbox" style="width: 20px; height: 20px; cursor: pointer;"><label for="new_auto_add_checkbox" style="font-weight: 600; font-size: 0.9rem; cursor: pointer; color: var(--primary-text-color);">${lang.lbl_auto_add}</label>
          </div>
          <div id="new_auto_add_subrow" style="display: flex; flex-direction: row; gap: 12px; width: 100%; transition: opacity 0.2s; opacity: 0.5;">
            <div class="edit-field" style="flex: 1;"><label>${lang.lbl_min_qty}</label><input type="number" id="new_min_qty" disabled placeholder="${lang.lbl_min_qty_placeholder}"></div>
            
            <div class="edit-field" style="flex: 1;">
              <label>${lang.lbl_todo}</label>
              <select id="new_todo" class="placeholder-style" disabled onchange="this.className = this.value === '' ? 'placeholder-style' : ''" style="width: 100%; padding: 6px 8px; border: 1px solid var(--divider-color); border-radius: 6px; background: var(--card-background-color); color: var(--primary-text-color); height: 34px;">
                <option value="">${lang.lbl_todo_placeholder}</option>
                ${todoLists.map(t => `${t.name}</option>`).join('')}
              </select>
            </div>
            
            <div class="edit-field" style="flex: 1;"><label>${lang.lbl_todo_placement}</label><select id="new_todo_placement" disabled style="width: 100%; padding: 6px 8px; border: 1px solid var(--divider-color); border-radius: 6px; background: var(--card-background-color); color: var(--primary-text-color); height: 34px;"><option value="name">${lang.opt_placement_name}</option><option value="description">${lang.opt_placement_desc}</option></select></div>
          </div>
        </div>
      </div>
      <div class="edit-form-actions" style="margin-top: 16px; border-top: 1px solid var(--divider-color); padding-top: 12px;"><button class="btn-cancel-add">${lang.btn_cancel}</button><button class="btn-save-add">${lang.btn_add}</button></div>
    </div>
  `;
}
